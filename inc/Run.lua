...
user.Source  = DirFol
local url , res = https.request('https://api.th3jm.com/GetID/?Array='..JSON.encode(user))
if res ~= 200 then
print(res)
...
jeam = Token:match("(%d+)")
redis:mset(
jeam..":VERSION",GetUser.information.Source_version,
jeam..":SUDO_ID:",GetUser.information.id,
jeam..":DataCenter:",GetUser.information.DataCenter,
jeam..":UserNameBot:",BOT_User,
jeam..":ApiSource",GetUser.information.WebSite,
jeam..":NameBot:","جيم",
"Th3jeam_INSTALL","Yes"
)
redis:hset(jeam..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {} 
...
jeam = Token:match("(%d+)")
our_id = tonumber(jeam)
Apijeam = redis:get(jeam..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(jeam..":UserNameBot:")
SUDO_ID = tonumber(redis:get(jeam..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_ID =  tonumber(redis:get(jeam..":SUDO_ID:"))
SUDO_USER = redis:hgetall(jeam..'username:'..SUDO_ID).username
version = redis:get(jeam..":VERSION")
DataCenter = redis:get(jeam..":DataCenter:")
...
\__$$  |$$  __$$\ $$$\    $$$ |
   ...
   $$ |   $$ |  $$ |$$ |\$  /$$ |
   $$ |   $$ |  $$ |$$ | \_/ $$ |
   \|   \__|  \__|\__|     \__|              ¦ VERSION » v]]..version..[[

-------------------------------------------------------------------
                                                  
]]..'\027[0;32m'
..'¦ TOKEN_BOT: \27[1;34m'..Token..'\027[0;32m\n'
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..jeam..')\027[0;32m\n'
...
..'¦ Api_Src->: \27[1;34m'..Apijeam..'\027[0;32m\n'
...
if not redis:get(jeam..":Check_Bot:"..W) then
Rgz,res=https.request(Apijeam..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(jeam..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(jeam..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(jeam..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(jeam..'group:add'..msg.chat_id_) then 
...
if msg.GroupActive and redis:get(jeam..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end
...
msg.Rank = 1
elseif redis:sismember(jeam..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(jeam..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ اساسي'
msg.TheRank = 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(jeam..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(jeam..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(jeam..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(jeam..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(jeam..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(jeam..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(jeam..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(jeam..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(jeam..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(jeam..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(jeam.."lock_Add"..msg.chat_id_) then
...
if msg.Special or msg.Admin then redis:srem(jeam..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(jeam..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(jeam..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.jeam then 
if msg.text and ScriptFile.ijeam then
	Xjeam = ScriptFile.jeam
	local list = redis:hgetall(jeam..":AwamerBotArray:"..msg.chat_id_)
	for jeam2,k in pairs(list) do
		Text = msg.text
		Text2 = k
		if Text:match(jeam2) then 
		local AmrOld = redis:hgetall(jeam..":AwamerBotArray2:"..msg.chat_id_)
		amrnew = "" amrold = ""
		...
		GetMsg = ScriptFile.ijeam(msg,{Text:match(Text2)})
		if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(jeam) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.ijeam(msg,{Text:match(jeam)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",jeam.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.djeam then
if ScriptFile.djeam(msg) == false then
return false
end
...
if Plug.jeam then 
if msg.text and Plug.ijeam then
for k, jeam in pairs(Plug.jeam) do
if msg.text:match(jeam) then
local GetMsg = Plug.ijeam(msg,{msg.text:match(jeam)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",jeam.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source jeam")
end 

end
else
print("The File Script.lua Not Runing in The Source jeam")
end
end
...
function tdcli_update_callback(data)
	...
	redis:srem(jeam..'users',msg.sender_user_id_)
	end
...
if redis:get(jeam..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(jeam..":propin"..msg.chat_id_)
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) vardump(d)end,nil)   
	...
	if redis:get(jeam..'group:add'..data.chat_.id_) then
	redis:set(jeam..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	elseif data.ID == "UpdateChannel" then  
	if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
	if redis:get(jeam..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(jeam..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(jeam..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	jeam..'group:name'..k,IDS.Title,
	jeam..'num_msg_max'..k,5,
	jeam..'group:add'..k,true,
	jeam..'lock_link'..k,true,
	jeam..'lock_spam'..k,true,
	jeam..'lock_webpage'..k,true,
	jeam..'lock_markdown'..k,true,
	jeam..'lock_flood'..k,true,
	jeam..'lock_bots'..k,true,
	jeam..'mute_forward'..k,true,
	jeam..'mute_contact'..k,true,
	jeam..'mute_document'..k,true,
	jeam..'mute_inline'..k,true,
	jeam..'lock_username'..k,true,
	jeam..'replay'..k,true
	)
	redis:sadd(jeam..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(jeam..'username:'..ID,'username',user)
	redis:sadd(jeam..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(jeam..'username:'..ID,'username',user)
	redis:sadd(jeam..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(jeam..'username:'..ID,'username',user)
	redis:sadd(jeam..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(jeam..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(jeam..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(jeam..'username:'..data.user_.id_).username
	...
	redis:hset(jeam..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
