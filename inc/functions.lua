...
download_file('https://raw.githubusercontent.com/TH3jJEAM/jeam/master/inc/Run.lua','./inc/Run.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'40% - |█████       |') end
download_file('https://raw.githubusercontent.com/TH3jeam/jeam/master/inc/locks.lua','./inc/locks.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'60% - |███████     |') end
download_file('https://raw.githubusercontent.com/TH3jeam/jeam/master/inc/Script.lua','./inc/Script.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'80% - |█████████   |') end
download_file('https://raw.githubusercontent.com/TH3jeam/jeam/master/inc/functions.lua','./inc/functions.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'100% - |█████████████|\n\n🔝*¦* تم تحديث السورس الى اصدار *v'..redis:get(jeam..":VERSION")..'*\n📟*¦* تم اعاده تشغيل السورس بنجاح') end
if edit then dofile("./inc/Run.lua") end
print("Update Source And Reload ~ ./inc/Run.lua")

end
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(jeam..":NameBot:") or "جيم"
...
var = 'مطور البوت 👨🏽‍💻' 
elseif redis:sismember(jeam..':MONSHA_Group:'..ChatID,UserID) then
var = ' المنشىء اساسي👷🏽' 
elseif redis:sismember(jeam..':MONSHA_BOT:'..ChatID,UserID) then
var = ' المنشىء 👷🏽' 
elseif redis:sismember(jeam..'owners:'..ChatID,UserID) then
var = 'مدير البوت 👨🏼‍⚕️' 
elseif redis:sismember(jeam..'admins:'..ChatID,UserID) then
var = 'ادمن في البوت 👨🏼‍🎓' 
elseif redis:sismember(jeam..'whitelist:'..ChatID,UserID) then
var = 'عضو مميز ⭐️' 
else
var = 'فقط عضو 🙍🏼‍♂️' 
end
...
if msg.edited then
redis:incr(jeam..':edited:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.text and not msg.forward_info_ then
redis:incr(jeam..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_)
elseif msg.content_.ID == "MessageChatAddMembers" then 
redis:incr(jeam..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessagePhoto" then
redis:incr(jeam..':photo:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageSticker" then
redis:incr(jeam..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVoice" then
redis:incr(jeam..':voice:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAudio" then
redis:incr(jeam..':audio:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVideo" then
redis:incr(jeam..':video:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAnimation" then
redis:incr(jeam..':animation:'..msg.chat_id_..':'..msg.sender_user_id_)
end
end
...
function TagAll(msg)
  ...
  local monshaas = redis:smembers(jeam..':MONSHA_Group:'..msg.chat_id_)
  local monsha = redis:smembers(jeam..':MONSHA_BOT:'..msg.chat_id_)
  local Owners = redis:smembers(jeam..'owners:'..msg.chat_id_)
  local Admins = redis:smembers(jeam..'admins:'..msg.chat_id_)
  local mmez = redis:smembers(jeam..'whitelist:'..msg.chat_id_)
...
for k,v in pairs(mmez) do
  if not message:match(v) then
    local info  = redis:hgetall(jeam..'username:'..v)
...
local list = redis:smembers(jeam..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* '..SUDO_USER..' ➣ (' ..SUDO_ID.. '){'..redis:scard(jeam..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info  = redis:hgetall(jeam..'username:'..v)
local count = redis:scard(jeam..'mtwr_count'..v)
message = message ..k.. '-l ['..(info.username or '')..'] l » (`' ..v.. '`){'..count..'} \n'
--redis:del(jeam..'username:'..v)
...
function conslist(msg)
    ...
    local monsha = redis:smembers(jeam..':MONSHA_Group:'..msg.chat_id_)
    if #monsha == 0 then 
...
local list = redis:smembers(jeam..'owners:'..msg.chat_id_)
if #list == 0 then  
...
local list = redis:smembers(jeam..'admins:'..msg.chat_id_)
...
local list = redis:smembers(jeam..'whitelist:'..msg.chat_id_)
...
if redis:sismember(jeam..'is_silent_users:'..Group,User) then 
return true 
else
return false
end 
...
local list = redis:smembers(jeam..'is_silent_users:'..msg.chat_id_)
...
if redis:sismember(jeam..'banned:'..Group,User) then 
return true 
else
return false
end 
...
if redis:sismember(jeam..'gban_users',User) then 
return true 
else
return false
end 
...
local list = redis:smembers(jeam..'gban_users')
...
function FilterX(msg,text)
  text = tostring(text)
  local var = false
local list = redis:smembers(jeam..':Filter_Word:'..msg.chat_id_)
if #list ~=0 then
...
if redis:sismember(jeam..':Filter_Word:'..msg.chat_id_,word) then 
return  "📝*¦* الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع✓️" 
else
redis:sadd(jeam..':Filter_Word:'..msg.chat_id_,word) 
...
if redis:sismember(jeam..':Filter_Word:'..msg.chat_id_,word) then 
redis:srem(jeam..':Filter_Word:'..msg.chat_id_,word) 
...
local NumberFlood = tonumber(redis:get(jeam..':Flood_Spam:'..User..':'..ChatID..':msgs') or 0)
if NumberFlood >= 5 then 
result = false
else
redis:setex(jeam..':Flood_Spam:'..User..':'..ChatID..':msgs',2,NumberFlood+1)
result = true
end
return result
end
...
json_data = '{"BotID": '..jeam..',"UserBot": "'..Bot_User..'","Groups" : {'
local All_Groups_ID = redis:smembers(jeam..'group:ids')
for key,GroupS in pairs(All_Groups_ID) do
local NameGroup = (redis:get(jeam..'group:name'..GroupS) or '')
...
local admins = redis:smembers(jeam..'admins:'..GroupS)
if #admins ~= 0 then
json_data =  json_data..',"Admins" : {'
for key,value in pairs(admins) do
local info = redis:hgetall(jeam..'username:'..value)
if info then 
...
local creator = redis:smembers(jeam..':MONSHA_BOT:'..GroupS)
if #creator ~= 0 then
...
local owner = redis:smembers(jeam..'owners:'..GroupS)
if #owner ~= 0 then
...
local list = redis:smembers(jeam..'group:ids')
message = '📋*¦* قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
local info = redis:get(jeam..'group:name'..v)
if info then 
...
redis:del(
jeam..'group:add'..id_group,
jeam..'lock_link'..id_group, 
jeam..'lock_id'..id_group,
jeam..'lock_spam'..id_group, 
jeam..'lock_webpage'..id_group,
jeam..'lock_markdown'..id_group,
jeam..'lock_flood'..id_group,
jeam..'lock_bots'..id_group,
jeam..'mute_forward'..id_group,
jeam..'mute_contact'..id_group,
jeam..'mute_location'..id_group,
jeam..'mute_document'..id_group,
jeam..'mute_keyboard'..id_group,
jeam..'mute_game'..id_group,
jeam..'mute_inline'..id_group,
jeam..'lock_username'..id_group,
jeam..'num_msg_max'..id_group,
jeam..'mute_text'..id_group,
jeam..'admins:'..id_group,
jeam..':Filter_Word:'..id_group,
jeam..'banned:'..id_group,
jeam..'is_silent_users:'..id_group,
jeam..'whitelist:'..id_group,
jeam..':MONSHA_BOT:'..id_group,
jeam..'owners:'..id_group,
jeam..'replay'..id_group,
jeam..':MONSHA_Group:'..id_group
)
redis:srem(jeam..'group:ids',id_group)
end
...
NumAdmin = NumAdmin + 1
if not redis:sismember(jeam..'admins:'..arg.chat_id_,v.user_id_) then
GetUserID(v.user_id_,function(arg,data)
redis:hset(jeam..'username:'..data.id_,'username',ResolveUserName(data))
redis:sadd(jeam..'admins:'..arg.chat_id_,data.id_)
...
if redis:get(jeam..'group:add'..msg.chat_id_) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
local GroupUsers = tonumber(redis:get(jeam..':addnumberusers') or 0)
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(jeam..'username:'..data.id_,'username', ResolveUserName(data))
redis:sadd(jeam..':MONSHA_Group:'..arg.chat_id_,data.id_)
...
redis:mset(
jeam..'group:add'..arg.chat_id_,true,
jeam..'lock_link'..arg.chat_id_,true, 
jeam..'lock_id'..arg.chat_id_,true,
jeam..'lock_spam'..arg.chat_id_,true,
jeam..'lock_webpage'..arg.chat_id_,true,
jeam..'lock_markdown'..arg.chat_id_,true,
jeam..'lock_flood'..arg.chat_id_,true,
jeam..'lock_bots'..arg.chat_id_,true,
jeam..'mute_forward'..arg.chat_id_,true,
jeam..'mute_contact'..arg.chat_id_,true,
jeam..'mute_location'..arg.chat_id_,true,
jeam..'mute_document'..arg.chat_id_,true,
jeam..'mute_keyboard'..arg.chat_id_,true,
jeam..'mute_game'..arg.chat_id_,true,
jeam..'mute_inline'..arg.chat_id_,true,
jeam..'lock_username'..arg.chat_id_,true,
jeam..'num_msg_max'..arg.chat_id_,5, 
jeam..'lock_edit'..arg.chat_id_,true,
jeam..'replay'..arg.chat_id_,true,
jeam.."lock_KickBan"..msg.chat_id_,true,
jeam.."lock_mmno3"..msg.chat_id_,true,
jeam.."lock_linkk"..msg.chat_id_,true
)
redis:sadd(jeam..'group:ids',arg.chat_id_) 
redis:sadd(jeam..'mtwr_count'..arg.sender_user_id_,arg.chat_id_)

local NameGroup = data.title_
redis:set(jeam..'group:name'..arg.chat_id_,NameGroup)
...
redis:set(jeam..'linkGroup'..arg.chat_id_,Gp_Link)
...
if cmd == "Upmonsh" then
if redis:sismember(jeam..':MONSHA_Group:'..ChatID,UserID) then 
...
redis:sadd(jeam..':MONSHA_Group:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮 » تم رفع ❲ "..NameUser.." ❳ منشئ اساسي في المجموعة \n✓️") 
end

if cmd == "Dwmonsh" then
if not redis:sismember(jeam..':MONSHA_Group:'..ChatID,UserID) then 
sendMsg(ChatID,MsgID,"📮 » تم بالتأكيد تنزيل ❲ "..NameUser.." ❳ منشئ اساسي في المجموعة \n✓️") 
end
redis:srem(jeam..':MONSHA_Group:'..ChatID,UserID) 
...
redis:hset(jeam..'username:'..UserID, 'username', Resolv)
redis:sadd(jeam..'banned:'..ChatID,UserID)
...
redis:hset(jeam..'username:'..UserID, 'username', Resolv)
redis:srem(jeam..'banned:'..ChatID,UserID)
StatusLeft(ChatID,UserID)
return false
end
...
redis:hset(jeam..'username:'..UserID, 'username', Resolv)
redis:sadd(jeam..'is_silent_users:'..ChatID,UserID)
...
redis:hset(jeam..'username:'..UserID, 'username', Resolv)
redis:srem(jeam..'is_silent_users:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📌 » العضو ❲ "..NameUser.." ❳  تم الغاء كتمه من المجموعة\n✓") 
end

if cmd == "upMshrf" then
  redis:hset(jeam..'username:'..UserID,'username',Resolv)
  redis:setex(jeam..":uploadingsomeon:"..ChatID..msg.sender_user_id_,500,NameUser)
  redis:setex(jeam..":uploadingsomeon2:"..ChatID..msg.sender_user_id_,500,UserID)
...
if cmd == "tfa3l" then  
  local maseegs = redis:get(jeam..'msgs:'..UserID..':'..ChatID) or 1
  local edited = redis:get(jeam..':edited:'..ChatID..':'..UserID) or 0
  local content = redis:get(jeam..':adduser:'..ChatID..':'..UserID) or 0
...
.."\n📝¦ التعديل » "..(redis:get(jeam..'lock_edit'..msg.chat_id_) or 'false')
.."\n🔗¦ الروابط » "..(redis:get(jeam..'lock_link'..msg.chat_id_) or 'false')
.."\n#️⃣¦ التاك » "..(redis:get(jeam..'lock_tag'..msg.chat_id_) or 'false')
.."\n©¦ المعرفات » "..(redis:get(jeam..'lock_username'..msg.chat_id_) or 'false')
.."\n\n💱¦ التكرار » "..(redis:get(jeam..'lock_flood'..msg.chat_id_) or 'false')
.."\n📑¦ الكلايش » "..(redis:get(jeam..'lock_spam'..msg.chat_id_) or 'false')
.."\n🌐¦ الويب » "..(redis:get(jeam..'lock_webpage'..msg.chat_id_) or 'false')
.."\n⚜️¦ الماركدوان » "..(redis:get(jeam..'lock_markdown'..msg.chat_id_) or 'false')
.."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(jeam..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖¦ البوتات » "..(redis:get(jeam..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕¦ عدد التكرار » "..(redis:get(jeam..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n🎬¦` اعدادات الوسائط :`\n"
.."\n🤹*¦* المتحركه » "..(redis:get(jeam..'mute_gif'..msg.chat_id_) or 'false')
.."\n💭¦ الدردشه » "..(redis:get(jeam..'mute_text'..msg.chat_id_) or 'false')
.."\n◽️¦ الانلاين » "..(redis:get(jeam..'mute_inline'..msg.chat_id_) or 'false')
.."\n🎮¦ الالعاب » "..(redis:get(jeam..'mute_game'..msg.chat_id_) or 'false')
.."\n🏞¦ الصور » "..(redis:get(jeam..'mute_photo'..msg.chat_id_) or 'false')
.."\n🎥¦ الفيديو » "..(redis:get(jeam..'mute_video'..msg.chat_id_) or 'false')
.."\n🎙¦ البصمات » "..(redis:get(jeam..'mute_audio'..msg.chat_id_) or 'false')
.."\n\n🔉¦ الصوت » "..(redis:get(jeam..'mute_voice'..msg.chat_id_) or 'false')
.."\n🎎¦ الملصقات » "..(redis:get(jeam..'mute_sticker'..msg.chat_id_) or 'false')
.."\n📞¦ الجهات » "..(redis:get(jeam..'mute_contact'..msg.chat_id_) or 'false')
.."\n💱¦ التوجيه » "..(redis:get(jeam..'mute_forward'..msg.chat_id_) or 'false')
.."\n🌐¦ الموقع » "..(redis:get(jeam..'mute_location'..msg.chat_id_) or 'false')
.."\n🗂¦ الملفات » "..(redis:get(jeam..'mute_document'..msg.chat_id_) or 'false')
.."\n🔅¦ الاشعارات » "..(redis:get(jeam..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n🔒¦ الفشار » "..(redis:get(jeam..'lock_mmno3'..msg.chat_id_) or 'false')
.."\n🔒¦ الفارسيه » "..(redis:get(jeam..'lock_pharsi'..msg.chat_id_) or 'false')
.."\n🔒¦ الانكليزيه » "..(redis:get(jeam..'lock_lang'..msg.chat_id_) or 'false')
.."\n🔒¦ الاضافه » "..(redis:get(jeam..'lock_join'..msg.chat_id_) or 'false')

local eueuf = "\n\n*⚒¦*` اعدادات اخرى : `"
.."\n*🙋🏼‍♂️¦* الترحيب » "..(redis:get(jeam..'welcome:get'..msg.chat_id_) or 'false')
.."\n*💬¦*  الردود » "..(redis:get(jeam..'replay'..msg.chat_id_) or 'false')
.."\n*🚸¦*  التحذير » "..(redis:get(jeam..'lock_woring'..msg.chat_id_) or 'false')
.."\n*🎫¦* الايدي » "..(redis:get(jeam..'lock_id'..msg.chat_id_) or 'false')
.."\n*🎫¦* الرابط » "..(redis:get(jeam..'lock_linkk'..msg.chat_id_) or 'false')
.."\n*🎫¦* المغادره » "..(redis:get(jeam..'lock_leftgroup'..msg.chat_id_) or 'false')
.."\n*🎫¦* الحظر » "..(redis:get(jeam..'lock_KickBan'..msg.chat_id_) or 'false')
.."\n*🎫¦* الحمايه » "..(redis:get(jeam..'antiedit'..msg.chat_id_) or 'false')
.."\n*🎫¦* التاك للكل » "..(redis:get(jeam..'lock_takkl'..msg.chat_id_) or 'false')
.."\n*🎫¦* الايدي بالصوره » "..(redis:get(jeam..'idphoto'..msg.chat_id_) or 'false')
.."\n*🎫¦* التحقق » "..(redis:get(jeam.."lock_check"..msg.chat_id_) or 'false')
...
.."\n*➕¦* عدد التكرار » "..(redis:get(jeam..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n*💱¦*` اعدادات التقـييد :`\n"
.."\n*💢¦* التقييد بالتوجيه » "..(redis:get(jeam..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*📸¦* التقييد بالصور » "..(redis:get(jeam..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*🔗¦* التقييد بالروابط » "..(redis:get(jeam..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*🎉¦* التقييد بالمتحركه » "..(redis:get(jeam..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*🎥¦* التقييد الفيديو » "..(redis:get(jeam..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
...
.."\n*🗂¦* الملفات » "..(redis:get(jeam..'mute_document'..msg.chat_id_) or 'false')
.."\n*🔅¦* الاشعارات » "..(redis:get(jeam..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n*🔒¦* الكيبورد » "..(redis:get(jeam..'mute_keyboard'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
return sendMsg(msg.chat_id_,msg.id_,'➖\n'..list_settings..'\n')
end
